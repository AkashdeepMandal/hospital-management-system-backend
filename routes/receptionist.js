...
router.get("/api/patient/getPatientName/:regNo", (req, res, next) => {
  ...
  //console.log(req.params.regNo)
  Patient.findOne(
    { patientRegistrationNumber: req.params.regNo },
...
router.get("/api/doctor/getNewRegNumber", (req, res, next) => {
  ...
  Doctor.find(null, "doctorRegistrationNumber")
    .sort("-doctorRegistrationNumber")
    .limit(1)
    .then((result) => {
...
router.get("/api/patients/:keyword", (req, res, next) => {
  ...
  Patient.find({
    $or: [
      { "name.firstname": new RegExp("^" + req.params.keyword, "i") },
      { "name.lastname": new RegExp("^" + req.params.keyword, "i") },
    ],
  })
    .then((results) => {
      res.status(200).json({
...
router.get("/api/findDoctors/:keyword", (req, res, next) => {
  ...
  Doctor.findOne({ doctorRegistrationNumber: Number(req.params.keyword) })
    .then((results) => {
      //console.log(results)
      res.status(200).json({
        doctor: results,
...
router.post("/api/doctor/updateAvailability", (req, res, next) => {
  ...
  DoctorAvailability.updateOne(
    { doctorRegistrationNumber: Number(req.body.doctorRegistrationNumber) },
...
router.get("/api/patient/getNewRegNumber", (req, res, next) => {
  ...
  Patient.find(null, "patientRegistrationNumber")
    .sort("-patientRegistrationNumber")
    .limit(1)
    .then((result) => {
...
router.get("/api/appointment/getNewNumber", (req, res, next) => {
  ...
  Appointment.find(null, "appointmentNumber")
...
router.get("/api/onlineappointment/getNewNumber", (req, res, next) => {
  console.log(
    "This is receptionist - Getting new Online appointment number route"
  );
  OnlineAppointment.find(null, "appointmentNumber")
    .sort("-appointmentNumber")
    .limit(1)
    .then((result) => {
...
router.post(
  "/api/appointment/getAppointmentCount_ScheduleAppointment",
  (req, res, next) => {
...
router.get("/api/doctors/getdoctorNames", (req, res, next) => {
  ...
  Doctor.find(null, "name doctorRegistrationNumber")
    .then((results) => {
      //console.log(results);
      res.status(200).json({
...
router.get(
  "/api/doctors/getdoctorAvailability/:doctorRegistrationNumber",
  (req, res, next) => {
    //console.log(req.params.firstName);
    DoctorAvailability.findOne({
      doctorRegistrationNumber: Number(req.params.doctorRegistrationNumber),
    })
      .then((results) => {
        //  console.log(results);
        res.status(200).json({
...
router.get("/api/admission/getNewNumber", (req, res, next) => {
  ...
  Admission.find(null, "admissionNumber")
...
router.get(
  "/api/admission/getAdmissionDetail/:admissionNumber",
  (req, res, next) => {
...
router.post("/api/admission/dischargePatient", (req, res, next) => {
  ...
  // console.log(req.body);
  Admission.updateOne(
    { admissionNumber: Number(req.body.admissionNum) },
    {
      $set: {
        status: "Discharged",
    ...
    .then((result) => {
      // console.log(result);
      Room.updateOne(
        { roomNumber: Number(req.body.roomNum) },
        { status: "Vacant" }
      ).then((re) => {
        //  console.log(re);
        res.status(201).json({});
...
router.post("/api/admission/viewAdmissionHistory", (req, res, next) => {
  console.log(
    "This is View Admission History - Patient Admissions - Receptionist"
  );
  Admission.aggregate([
...
router.get("/api/onlineAppointments/getDoctorList", (req, res, next) => {
  ...
  Doctor.find(null, "name doctorRegistrationNumber SLMCRegNo doctorType")
    .then((results) => {
      //console.log(results);
      res.status(200).json({
        doctors: results,
...
router.get(
  "/api/onlineAppointments/searchDoctor/:keyword",
  (req, res, next) => {
...
router.post("/api/onlineAppointments/viewByDoctor", (req, res, next) => {
  console.log(
    "This is Viewing Online Appointments - View By Doctor - Receptionist"
  );
...
router.post(
  "/api/onlineAppointments/viewByDoctor_LinkPatient",
  (req, res, next) => {
    console.log(
      "This is Viewing Online Appointments for Linking Patient- View By Doctor - Receptionist"
    );
...
router.post("/api/onlineAppointments/linkPatient", (req, res, next) => {
  ...
  // console.log(req.body);
  OnlineAppointment.updateOne(
    { _id: req.body.patientID },
    {
      $set: {
        appointmentStatus: "Linked",
        patientRegistrationNumber: Number(req.body.patientRegistrationNumber),
      },
    }
  )
    .then((results) => {
...
router.get("/api/dashboard/countNormalAppointments", (req, res, next) => {
  console.log(
    "This is getting number of normal appointments - Receptionist Dashboard"
  );
...
router.get("/api/dashboard/countOnlineAppointments", (req, res, next) => {
  console.log(
    "This is getting number of online appointments - Receptionist Dashboard"
  );
